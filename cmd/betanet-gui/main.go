...
func (s *nodeState) startNode(logArea *widget.Entry, data, listen, bootstrap string) error {
	...
	node, err := p2p.New(ctx, db, listen, splitCSV(bootstrap), nil)
	if err != nil {
		cancel()
		db.Close()
		return err
...
func buildNodeTab(win fyne.Window) fyne.CanvasObject {
	...
	startBtn := widget.NewButton("Start Node", func() {
		data := dataEntry.Text
		if data == "" {
			data = filepath.Join(os.Getenv("HOME"), ".betanet", "node")
		}
...
func buildWalletTab(win fyne.Window) fyne.CanvasObject {
	...
	pickWalletBtn := widget.NewButton("Select Wallet", func() {
		dlg := dialog.NewFileOpen(func(uri fyne.URIReadCloser, err error) {
			if err == nil && uri != nil {
				walletPath.SetText(uri.URI().Path())
			}
		}, win)
		dlg.Show()
	})
	newWalletBtn := widget.NewButton("New Wallet", func() {
		...
		if path == "" {
			path = filepath.Join(os.Getenv("HOME"), ".betanet", "wallet.json")
		}
	...
	addSiteBtn := widget.NewButton("Add Site", func() {
		...
		if err := saveWalletPath(walletPath.Text, w, mnemonic.Text); err != nil {
			dialog.ShowError(err, win)
			return
		}
	...
	pickContentBtn := widget.NewButton("Select Content", func() {
		dlg := dialog.NewFileOpen(func(r fyne.URIReadCloser, err error) {
			if err == nil && r != nil {
				contentPath.SetText(r.URI().Path())
			}
		}, win)
		dlg.Show()
	})
	...
	publishBtn := widget.NewButton("Publish", func() {
		...
		defer cancel()
		node, err := p2p.New(ctx, db, listen.Text, splitCSV(bootstrap.Text), nil)
		if err != nil {
			db.Close()
			dialog.ShowError(err, win)
			return
		...
		if err := db.PutRecord(recCID, env.Record); err != nil {
			db.Close()
			dialog.ShowError(err, win)
			return
		}
		if len(env.Content) > 0 {
			if err := db.PutContent(core.CIDForContent(env.Content), env.Content); err != nil {
				db.Close()
				dialog.ShowError(err, win)
				return
			}
		}
		if err := db.SetHead(siteID, seq, recCID); err != nil {
			db.Close()
			dialog.ShowError(err, win)
			return
		}
		if err := node.BroadcastUpdate(ctx, *env); err != nil {
			db.Close()
			dialog.ShowError(err, win)
			return
		}
	...
	deleteBtn := widget.NewButton("Delete (record/content)", func() {
		...
		if err := node.BroadcastDelete(ctx, del); err != nil {
			dialog.ShowError(err, win)
			return
		}
	...
	fetchBtn := widget.NewButton("Fetch Content", func() {
		if strings.TrimSpace(fetchCID.Text) == "" {
			dialog.ShowInformation("Fetch", "Provide a content CID or prefix", win)
			return
		}
		data := nodeData.Text
		if data == "" {
		...
		if len(cid) < 64 {
			if full, err := db.ResolveContentCID(cid); err == nil {
				cid = full
			}
		}
		...
		if err := os.WriteFile(fetchOutPath.Text, b, 0644); err != nil {
			dialog.ShowError(err, win)
			return
		}
...
func openWalletPath(path, mnemonic string) (*wallet.Wallet, []byte, error) {
	if path == "" {
		return nil, nil, fmt.Errorf("wallet path required")
	}
